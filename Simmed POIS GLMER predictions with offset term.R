...
# Set some parameter for simulated data:
set.seed(seed = 222)
beta0 <- -3.45
beta.CWD <- 0.01
beta.basal.area <- 0.1
...
# Check these predictions manually:
...
# Alternative coding - offset as ARGUMENT, not as term in model - note that this avoids things like MuMIn having to 'handle'
#  the offset term somehow - as it is an argument it automatically gets included in all models fitted by dredge()!
...
# The problem is that because there is no 'term' for the offset, we can't easily specify what value to predict for... 
#  so we are not taking the 'effort' into account as per previous predictions.
# We can demonstrate this by doing the predictions manually instead.
# First make a prediction 'frame' (equivalent to newdata but need to be a matrix, and you need to explicitly code the intercept column, etc):
